pub mod auth_service {
    ...
    pub async fn register(args: AuthServicePayload) -> Result<(), (StatusCode, String)> {
        ...
        let password_hash = hash(args.payload.password, DEFAULT_COST).map_err(internarnal_error)?;
    ...
    pub async fn login(
        ...
        if let Some(user) = user {
            let check =
                verify(args.payload.password, &user.password_hash).map_err(internarnal_error)?;
